...
namespace CompanyHome.Controllers
{
    public class PageController : Controller
    {
        ...
        public IActionResult Index(int id = 1)//第一页不需要参数 所以规定p为空的时候 默认是1
        {
            ...
            {
                list = myDBContent.Pages.Take(pageSize).ToList();//=0的时候直接获取数据
            }
            else
            {
                list = myDBContent.Pages.Skip(skip).Take(pageSize).ToList();//>0时跳过xx页，取数据
            }
        ...
        {
            var page = myDBContent.Pages.FirstOrDefault(p => p.ID == id);
            ViewBag.Page = page;
            return View();
        }
    }
